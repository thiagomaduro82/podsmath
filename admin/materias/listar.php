...
<table class="table table-bordered table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from materias order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from materias where nome LIKE '$txtbuscar' or nomemenu LIKE '$txtbuscar' order by id desc");
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from materias");
